...
//!
//! The concrete route key type is therefore generic here and should come from
//! the route-transport side.
...
pub enum SizeHint {
    ...
    UpperBound(usize),
    /// Given value is an estimate that may be too larger or too small.
    Estimate(usize),
}
...
///
/// `R` is the concrete route key type owned by route transport/discovery.
