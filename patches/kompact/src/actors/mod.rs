...
/// Convenience actor API for components that only accept local messages.
///
/// Types implementing this trait automatically implement [Actor] with
/// `receive` as the only local-message handler. Pair it with
/// [`impl_local_actor!`](crate::impl_local_actor) to generate the corresponding
/// [ActorRaw] implementation for codebases that do not use Kompact's actor-level
/// network messaging path at all.
pub trait LocalActor {
    ...
    /// Handle an incoming local message.
    ///
    /// Local messages are of type `Self::Message`.
    fn receive(&mut self, msg: Self::Message) -> Handled;
}

/// Generates an [ActorRaw] implementation for a [`LocalActor`](crate::prelude::LocalActor).
///
/// This avoids repeating the same `MsgEnvelope` unpacking and unreachable
/// network-message branch on every local-only actor implementation.
#[macro_export]
macro_rules! impl_local_actor {
    ($ty:ty) => {
        impl $crate::prelude::ActorRaw for $ty {
            type Message = <$ty as $crate::prelude::LocalActor>::Message;

            #[allow(unreachable_code)]
            fn receive(
                &mut self,
                env: $crate::messaging::MsgEnvelope<Self::Message>,
            ) -> $crate::component::Handled {
                match env {
                    $crate::messaging::MsgEnvelope::Typed(msg) => {
                        <Self as $crate::prelude::LocalActor>::receive(self, msg)
                    }
                    $crate::messaging::MsgEnvelope::Net(_) => {
